    ...
    bundle.traverse(node => {
      if (node.type === 'dependency') {
        ...
        if (node.value.env.isServer() && resolvedAsset && Array.isArray(directives) && directives.includes('use client')) {
          ...
          let jsBundles = bundles.filter(b => b.type === 'js' && b.env.isBrowser()).map(b => (0, _utils().normalizeSeparators)(b.name));
          ...
          for (let symbol of bundleGraph.getExportedSymbols(resolvedAsset, bundle)) {
            let ref = `createClientReference(${JSON.stringify(bundleGraph.getAssetPublicId(symbol.asset))}, ${JSON.stringify(symbol.exportSymbol)}, ${JSON.stringify(jsBundles)})`;
            if (resources.length) {
          ...
          if ((asyncResolution === null || asyncResolution === void 0 ? void 0 : asyncResolution.type) === 'bundle_group') {
            ...
            for (let b of bundles) {
              if (b.type === 'css') {
                ...
                if (bundle.env.isBrowser()) {
                  ...
                  preinit.push(`preinit(${JSON.stringify(url)}, {as: 'style', precedence: 'default'});`);
                ...
                if (b.env.isBrowser()) {
                  ...
                  resources.push(`<script type="module" async src=${JSON.stringify(url)} />`);
            ...
            }
            {
              ...
              if (node.value.priority !== 'lazy' && entry) {
                let bootstrapScript = `Promise.all([${bootstrapModules.map(m => `import("${m}")`).join(',')}]).then(()=>${(0, _nullthrows().default)(config).parcelRequireName}(${JSON.stringify(bundleGraph.getAssetPublicId(entry))}))`;
              ...
              code += `let res = createResourcesProxy(originalModule, resources ${node.value.priority !== 'lazy' && entry ? ', bootstrapScript' : ''});\n`;
              if (node.value.priority === 'lazy') {
                code += `  return res;\n`;
                code += `});\n`;
...
function renderStylesheet(b) {
  let url = (0, _utils().urlJoin)(b.target.publicUrl, b.name);
  return `<link rel="stylesheet" href=${JSON.stringify(url)} precedence="default" />`;
}