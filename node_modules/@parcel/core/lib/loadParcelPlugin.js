...
async function loadPlugin(pluginName, configPath, keyPath, options) {
  ...
  if (resolveFrom.includes(NODE_MODULES) || "production" !== 'production' && /packages[/\\]configs/.test(resolveFrom)) {
    ...
    if (configPkg != null && ((_configPkg$config$dep = configPkg.config.dependencies) === null || _configPkg$config$dep === void 0 ? void 0 : _configPkg$config$dep[pluginName]) == null) {
      ...
      isOptional = Array.isArray(configPkg.config.optionalParcelDependencies) && configPkg.config.optionalParcelDependencies.includes(pluginName);
